...
if(isset($_POST['submit'])){
    if($_FILES['image']['name'])
    {
        ...
        $qry = mysqli_query($con, "update tbl_foodcategories set name='" . $_POST['cat'] . "',image='$imagepath' where id='" . $_POST['id'] . "' ");
        if ($qry) {
            ?>
        ...
        $qry = mysqli_query($con, "update tbl_foodcategories set name='" . $_POST['cat'] . "' where id='" . $_POST['id'] . "' ");
        if ($qry) {
            ?>
            <script>alert("Category Update Successfuly !!");
                window.location = 'addfoodcategori.php';</script><?php
...
if(isset($_POST['addcat'])){
    ...
    //Upload the file into the temp dir
    move_uploaded_file($tmpFilePath, $newFilePath);
    $qry=mysqli_query($con,"insert into tbl_foodcategories (`name`,`image`) VALUES ('".$_POST['cat']."','$imagepath') ");
    if($qry){
